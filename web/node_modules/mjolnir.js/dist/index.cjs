...
__export(index_exports, {
  DoubleClickDrag: () => DoubleClickDragRecognizer,
...
// dist/hammerjs/recognizers/double-click-drag.js
var EVENT_NAMES = ["", "start", "move", "end", "cancel"];
var DoubleClickDragRecognizer = class extends Recognizer {
  constructor(options = {}) {
    super({
      enable: true,
  ...
  emit(input) {
    ...
    if (this.state === RecognizerState.Began) {
      if (!((_a = this._drag) == null ? void 0 : _a.active) || this._emittedStart) {
        return;
      }
...
// dist/hammerjs/recognizers/trackpad.js
var TrackpadRecognizer = class extends AttrRecognizer {
  ...
  set(options) {
    ...
    if (wheelSession && wheelSession !== this.wheelSession) {
      (_a = this.wheelSessionUnsubscribe) == null ? void 0 : _a.call(this);
      this.wheelSessionUnsubscribe = null;
...
var EVENT_NAMES2 = ["", "start", "move", "end", "cancel", "up", "down", "left", "right"];
...
var EVENT_NAMES3 = ["", "up", "down", "left", "right"];
...
var EVENT_NAMES4 = ["", "start", "move", "end", "cancel", "in", "out"];
...
var EVENT_NAMES5 = ["", "start", "move", "end", "cancel"];
...
var PressRecognizer = class extends Recognizer {
  ...
  getEventNames() {
    return EVENT_NAMES6.map((suffix) => this.options.event + suffix);
  }
  process(input) {
    const { options } = this;
    const validPointers = input.pointers.length === options.pointers;
...
var WheelInput = class extends Input2 {
  constructor(element, callback, options) {
    ...
    this.handleEvent = (event) => {
      ...
      this.callback({
        ...
        delta: -value,
        device: ((_a2 = this.options.wheelSession) == null ? void 0 : _a2.device) ?? "unknown",
        srcEvent: event,
        pointerType: "mouse",
        target: event.target
      });
    ...
    if (options.enable) {
      this.wheelSessionUnsubscribe = (_a = this.options.wheelSession) == null ? void 0 : _a.on(() => {
      });
  ...
  enableEventType(eventType, enabled) {
    ...
    if (eventType === "wheel" && this.options.enable !== enabled) {
      ...
      if (!enabled) {
        (_b = this.wheelSessionUnsubscribe) == null ? void 0 : _b.call(this);
        this.wheelSessionUnsubscribe = void 0;
      }
    }
  }
...
// dist/inputs/wheel-gesture-session.js
var WHEEL_DELTA_MAGIC_SCALER = 4.000244140625;
var WHEEL_DELTA_PER_LINE2 = 40;
...
var WheelGestureSession = class {
  constructor(element, options = {}) {
    ...
    this.end = () => {
      ...
      if (this.session.device === "unknown") {
        ...
        const device = classifyWheelSession(session2.samples, true);
        this.begin(session2, device === "unknown" ? "mouse" : device);
      }
      if (!this.session) {
        return;
      }
    ...
    (_a = this.element) == null ? void 0 : _a.addEventListener("wheel", this.handleEvent, { passive: true });
  }
  get hasSubscribers() {
  ...
  get device() {
    var _a;
    return ((_a = this.session) == null ? void 0 : _a.device) ?? "unknown";
  }
  on(listener) {
  ...
  destroy() {
    ...
    this.updateControlKeyEventListeners();
    (_a = this.element) == null ? void 0 : _a.removeEventListener("wheel", this.handleEvent);
  }
  ...
  emit(eventType, srcEvent, velocities) {
    ...
    const velocityX = isFirst ? 0 : (velocities == null ? void 0 : velocities.velocityX) ?? session.velocityX;
    const velocityY = isFirst ? 0 : (velocities == null ? void 0 : velocities.velocityY) ?? session.velocityY;
    const event = {
      eventType,
...
function getWindow() {
  ...
  return (_a = globalThis.document) == null ? void 0 : _a.defaultView;
}
function createSample(event, isControlKeyDown) {
  let deltaX = event.deltaX;
  let deltaY = event.deltaY;
  if (event.deltaMode === DOM_DELTA_LINE) {
    deltaX *= WHEEL_DELTA_PER_LINE2;
    deltaY *= WHEEL_DELTA_PER_LINE2;
  }
  return {
    event,
    timeStamp: event.timeStamp,
...
var EventManager = class {
  ...
  emit(event) {
    var _a;
    (_a = this.manager) == null ? void 0 : _a.emit(event.type, event);
  }
  /*
